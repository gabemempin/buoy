...
**Known open issue.** `BuoyTextView` registers an I-beam `NSTrackingArea` that bleeds through SwiftUI overlay panels (SettingsPanel, AllNotesPanel). `ArrowCursorOverlay` in `WindowDragBlocker.swift` attempts to override it via `cursorUpdate(with:)` + `resetCursorRects`, applied as `.overlay(ArrowCursorOverlay().allowsHitTesting(false))`. This does NOT fully work: AppKit dispatches `cursorUpdate` only to the deepest hit-testable view, so the overlay (excluded from hit testing) is skipped and BuoyTextView wins. Attempted fixes: (1) removing `allowsHitTesting(false)` + forwarding mouse events via `nextResponder` — broke SwiftUI button hover/click routing entirely; (2) `NSEvent.addLocalMonitorForEvents(.cursorUpdate)` — reverted before confirming. The `NSEvent` monitor approach is the most promising direction for a future fix.
