...
- **Launch crash (`NSMutableRLEArray` out of bounds)** — `NoteStore+Migration.swift`: `parseInlineHTML` had dead code calling `stripped.attributes(at: 0, ...)` on a potentially empty `NSAttributedString`. Notes with `<br>`-only content (e.g. "The old FloatNotes", "divisons") caused it to crash every launch since their `contentRTF` was still empty and migration re-ran every time. Fixed by removing the dead code; `parseInlineHTML` now just strips HTML tags.
